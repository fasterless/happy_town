...
function getAchievementProgress(state, achievement) {
  ...
  if (achievement.trackKey === "yearbook_cover") {
    const bound = Object.values(state.explore?.volumes || {}).filter((volume) => volume && volume.bound).length;
    return [1, 3, 5].filter((need) => bound >= need).length;
  }
